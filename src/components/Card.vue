<template>
  ...
  >
    ...
    <div v-if="isFaceDown" class="w-full h-full flex items-center justify-center relative">
      ...
      <div :class="compact ? 'text-xl' : 'text-3xl'" class="text-gold font-bold relative z-10 drop-shadow-lg">?</div>
    </div>

    <!-- Front of Card -->
    <div v-else class="flex flex-col items-center">
      <span :class="[suitColor, compact ? 'text-xl leading-none' : 'text-xl font-bold font-mono']" class="font-bold">
        {{ displayValue }}
      </span>
      <span v-if="type === 'number' && !compact" class="text-xs uppercase font-bold opacity-60" :class="suitColor">
        {{ suit }}
      </span>
      <!-- Compact Suit Indicator (Small Dot/Icon) -->
      <span v-if="type === 'number' && compact" :class="suitColor" class="text-[8px] leading-none opacity-80">
